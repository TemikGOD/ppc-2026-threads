...
DenisovAQsortSimpleMergeTBB::DenisovAQsortSimpleMergeTBB(const InType &in) {
...
void DenisovAQsortSimpleMergeTBB::SimpleMerge(const std::vector<int> &source, std::vector<int> &destination,
...
void DenisovAQsortSimpleMergeTBB::SortBlockIfNeeded(std::vector<int> &data, size_t size, size_t block_index) {
  ...
  if (block_end > block_start + 1U && block_end <= size && block_start < size) {
    HoareSort(data, static_cast<int>(block_start), static_cast<int>(block_end - 1U));
  }
}

void DenisovAQsortSimpleMergeTBB::SortBlocks(std::vector<int> &data, size_t size) {
...
void DenisovAQsortSimpleMergeTBB::MergeChunk(const std::vector<int> &source, std::vector<int> &destination,
...
void DenisovAQsortSimpleMergeTBB::MergePass(std::vector<int> &data, size_t size, size_t merge_width) {
...
bool DenisovAQsortSimpleMergeTBB::ValidationImpl() {
  return !GetInput().empty();
}

bool DenisovAQsortSimpleMergeTBB::PreProcessingImpl() {
  data_ = GetInput();
  GetOutput().clear();
  return true;
}

bool DenisovAQsortSimpleMergeTBB::RunImpl() {
